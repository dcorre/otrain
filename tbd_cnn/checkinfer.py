...
class SortRes():
    ...
    def __init__(self, path_crossmatch, path_infer):
        ...
        self.data1 = ascii.read(os.path.join(path_crossmatch, 
                                             'crossmatch.dat'))
        self.data2 = ascii.read(os.path.join(path_infer,
                                             'infer_results.dat'))
    ...
    def combine_match_cnn(self, prob, colname):
        ...
        for row in self.data2:
            mask = (self.data1['closest_candID'] ==
                    row['cand_ID']) & (row[colname] > prob)
    ...
    def hist(self, data, col, bins=20):
        plt.hist(data[col], bins,  density=False,
                 facecolor='C0', alpha=0.75, log=True)
...
def makestats(path_plots, path_crossmatch, path_infer, maglim,
    ...
    _, _, _ = plt.hist(
        ...
        bins=bins,
        label='%.2f<%s<%.2f ' % (
            CNNproblim[0], colmask, CNNproblim[1]),
        color='C0',
        alpha=0.5,
        log=True)
    _, _, _ = plt.hist(
        ...
        bins=bins,
        label='%.2f<%s<%.2f' % (
            CNNproblim[1], colmask, CNNproblim[2]),
        color='C1',
        alpha=0.5)
    _, _, _ = plt.hist(
