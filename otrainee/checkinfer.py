...
class SortRes:
    def __init__(self, path_crossmatch, path_infer):
        # FIXME: might pass them as arguments for the user to use
        # different filenames in the future
        self.data1 = ascii.read(os.path.join(path_crossmatch, "crossmatch.dat"))
        self.data2 = ascii.read(os.path.join(path_infer, "infer_results.dat"))
    ...
    def combine_match_cnn(self, prob, colname):
        ...
        for row in self.data2:
            mask = (self.data1["closest_candID"] == row["cand_ID"]) & (
                row[colname] > prob
            )
...
def makestats(
    ...
    simID_list = res.combine_match_cnn(prob=0.0, colname="label1")
    ...
    mask2 = (simID_list[colmask] < CNNproblim[2]) & (
        simID_list[colmask] > CNNproblim[1]
    )
    mask3 = (simID_list[colmask] < CNNproblim[1]) & (
        simID_list[colmask] > CNNproblim[0]
    )
    ...
    _, _, _ = plt.hist(
        simID_list[coldistrib][mask3] / simID_list["FWHMPSF"][mask3],
        bins=bins,
    ...
    _, _, _ = plt.hist(
        simID_list[coldistrib][mask2] / simID_list["FWHMPSF"][mask2],
        bins=bins,
    ...
    _, _, _ = plt.hist(
        simID_list[coldistrib][mask1] / simID_list["FWHMPSF"][mask1],
        bins=bins,
    ...
    """
    _, _, _ = plt.hist(
        res.data2["FWHM"][~mask_sim] / res.data2["FWHMPSF"][~mask_sim],
        bins=bins,
        log=True,
        color="C0",
        alpha=0.4,
    ...
    _, _, _ = plt.hist(
        simID_list["FWHM"][mask] / simID_list["FWHMPSF"][mask],
        bins=bins,
